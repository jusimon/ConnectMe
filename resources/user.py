...
class QuizQuestion(Resource):
    ...
    def get(self):
        ...
        if data['querytype'].lower() == 'recruiter':
            ...
            query = "select qset_title,email_sent,client_email_id,invite_qset_status,a.created_by,invite_id from `eba_quiz_invites` as b,`eba_quiz_question_sets` as a where a.`qset_id`=b.`qset_id` and b.`client_email_id`='test2@email.com111'"
...
class EmailInvite(Resource):
    ...
    def get(self):
        ...
        parser.add_argument('created_by', type=str, required=False)
        parser.add_argument('tenancy_id', type=str, required=False)
        parser.add_argument('invite_id', type=str, required=False)
        parser.add_argument('querytype', type=str, required=True)
        data = parser.parse_args()
        print(data)
        try:
        ...
        if data['querytype'] == 'recruiter':
            ...
            argument = data['tenancy_id']
            cursor.execute(query,argument)
        elif data['querytype'] == 'getqsetid':
            query = "select qset_id from eba_quiz_invites where invite_id=%s"
            argument = data['invite_id']
            cursor.execute(query,argument)
        else:
